...
-- Experiment to see if RBMap is faster. Definitely not.
-- 93 sec instead of 10 sec
...
abbrev LList := RBMap Nat Node compare
...
def makeList (nums : Array Int) : RBMap Nat Node compare :=
  let n := nums.size
  let pairs : Array (Nat × Node) := 
    nums.mapIdx λ ix v => (ix, (Node.mk ((ix + n - 1) % n) ((ix+1) % n) v))
  RBMap.fromList pairs.toList compare
